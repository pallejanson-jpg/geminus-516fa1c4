...
import {
  ...
  Zap, Archive, Radar, Scan, Globe, Cuboid, LifeBuoy, GitMerge,
} from 'lucide-react';
...
export const SIDEBAR_ITEM_META: Record<string, SidebarItemMeta> = {
  ...
  geminus_base_native:       { icon: Building2,     color: 'text-accent',      label: 'Geminus Base 2.0',                            type: 'internal', contexts: ['global', 'building'] },
  geminus_plus:       { icon: Box,           color: 'text-primary',     label: DEFAULT_APP_CONFIGS.geminus_plus.label,  type: 'config',   contexts: ['building'] },
  ...
  ifc_federation:   { icon: GitMerge,      color: 'text-accent',      label: 'IFC Federation',                     type: 'internal', contexts: ['global', 'building'] },
};
