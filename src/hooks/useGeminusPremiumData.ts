...
export interface GeminusPremiumCurrentValues {
  temperature: number | null;
  co2: number | null;
  humidity: number | null;
  occupancy: number | null;
...
export interface GeminusPremiumMachineData {
  machinePk: number;
  machineName: string;
  machineLabel?: string;
  siteName?: string;
  lineName?: string;
  dashboardUrl: string;
  current: GeminusPremiumCurrentValues;
  timeSeries: GeminusPremiumTimePoint[];
  availableFields: string[];
}

// Parse Elasticsearch aggregation bucket response into usable time series
function parseTimeSeries(esData: any): GeminusPremiumTimePoint[] {
...
function parseMachineData(dataRows: any[]): GeminusPremiumTimePoint[] {
...
function deriveCurrentValues(timeSeries: GeminusPremiumTimePoint[], latestValues: any): GeminusPremiumCurrentValues {
  ...
  const result: GeminusPremiumCurrentValues = { temperature: null, co2: null, humidity: null, occupancy: null, light: null };
...
function detectAvailableFields(timeSeries: GeminusPremiumTimePoint[], properties: any[]): string[] {
...
function generateMockTimeSeries(fmGuid: string, days = 7): GeminusPremiumTimePoint[] {
  ...
  const pts: GeminusPremiumTimePoint[] = [];
  const now = new Date();
...
// ── Main hook ──
export function useGeminusPremiumData(fmGuid: string | null | undefined) {
  const [data, setData] = useState<GeminusPremiumMachineData | null>(null);
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const { data: result, error: fnError } = await supabase.functions.invoke('geminus-premium-query', {
        ...
        if (fnError || !result?.success) {
          ...
          try {
            const { data: urlData } = await supabase.functions.invoke('geminus-premium-query', {
          ...
          setIsLive(false);
          setError(fnError?.message ?? result?.error ?? 'Geminus Premium not available');
        } else {
        ...
        if (abortController.signal.aborted) return;
        console.error('[useGeminusPremiumData] Unexpected error:', err);
        const mockTs = generateMockTimeSeries(fmGuid);
        setData({
          machinePk: 0,
          machineName: fmGuid,
...
export interface GeminusPremiumBuildingData {
  siteName: string;
  ...
  useEffect(() => {
    ...
    supabase.functions
      ...
      .then(({ data: result, error: fnError }) => {
        if (fnError || !result?.success) {
          setError(fnError?.message ?? result?.error ?? 'No Geminus Premium data for this building');
          setIsLive(false);
        } else {
          setData({
