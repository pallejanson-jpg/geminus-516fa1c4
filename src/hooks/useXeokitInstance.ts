  ...
  const createInstance = useCallback(async (): Promise<XeokitInstanceResult | null> => {
    ...
    if (viewer.cameraControl) {
      ...
      const navTuning = isMobileRef.current
        ? { dragRotationRate: 70, rotationInertia: 0.88, touchPanRate: 0.14, panInertia: 0.82, touchDollyRate: 0.09, mouseWheelDollyRate: 35, keyboardDollyRate: 4 }
        : { dragRotationRate: 120 * speedMultiplier, rotationInertia: 0.85, touchPanRate: 0.3 * speedMultiplier, panInertia: 0.7, touchDollyRate: 0.15 * speedMultiplier, mouseWheelDollyRate: 50 * speedMultiplier, keyboardDollyRate: 5 * speedMultiplier };
