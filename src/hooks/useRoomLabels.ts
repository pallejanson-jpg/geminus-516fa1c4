...
import { buildRoomChildrenMap, computeRoomCentroid } from '@/lib/room-centroid';
...
export function useRoomLabels(
  ...
  const createLabels = useCallback((floorGuids?: string[]) => {
    ...
    // Build parent->children map once for geometry centroid calculation
    const labelChildrenMap = buildRoomChildrenMap(metaObjects);
    ...
    Object.values(metaObjects).forEach((metaObj: any) => {
      ...
      const centroid = computeRoomCentroid(viewer, metaObj, labelChildrenMap);
      const center = [centroid?.x ?? (aabb[0] + aabb[3]) / 2, labelHeight, centroid?.z ?? (aabb[2] + aabb[5]) / 2];
