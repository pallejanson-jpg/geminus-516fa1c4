 ...
 * This reads from the `assets` table which is populated by the geminus-plus-sync edge function.
 * 
...
/**
 * Fetch a flat list of objects from Geminus Plus API via edge function.
 ...
 */
export async function fetchGeminusPlusData(filter: GeminusPlusFilter): Promise<any[]> {
  const { data, error } = await supabase.functions.invoke("geminus-plus-query", {
    body: { filter },
  });

  if (error) {
    throw new Error(error.message || "Geminus Plus fetch failed");
  }
...
// Types for Geminus Plus object creation
...
/**
 * Create a new asset (ObjectType 4) in Geminus Plus via edge function.
 * The asset will be linked to the specified parent Space.
 */
export async function createGeminusPlusObject(payload: CreateAssetPayload): Promise<any> {
 ...
 * Automatically syncs to Geminus Plus for non-local assets (is_local = false).
 * 
...
export async function updateAssetProperties(
  ...
  const { data, error } = await supabase.functions.invoke("geminus-plus-update", {
...
/**
 * Update an existing asset in Geminus Plus.
 * @deprecated Use updateAssetProperties() instead for better batch support
 */
export async function updateGeminusPlus(payload: {
  fmGuid: string;
  properties: UpdatePropertyItem[];
}): Promise<UpdateAssetsResponse> {
...
/**
 * Sync a local asset to Geminus Plus API.
 * This pushes an asset that was created locally (is_local=true) to the external Geminus Plus system.
 * 
 * Requirements:
 * - Asset must have in_room_fm_guid (parent space) - required by Geminus Plus
 * - Asset should have a name/designation
 * 
 * @returns Success status and any error message
 */
export async function syncAssetToGeminusPlus(assetFmGuid: string): Promise<{ success: boolean; error?: string; asset?: any }> {
  ...
  // 3. Build properties array for Geminus Plus (only non-Lovable-specific fields)
  ...
  // 4. Call edge function to create in Geminus Plus
  ...
  // Include existing fmGuid so Geminus Plus uses the same ID
  const requestBody = {
    ...payload,
    fmGuid: asset.fm_guid,
  ...
  if (error) {
    return {
      success: false,
      error: error.message || "Failed to sync to Geminus Plus",
    };
  }
  ...
  if (!data?.success) {
    return {
      success: false,
      error: data?.error || "Unknown error syncing to Geminus Plus",
    };
  }
...
/**
 * Batch sync multiple local assets to Geminus Plus.
 * @returns Summary of results
 */
export async function batchSyncAssetsToGeminusPlus(assetFmGuids: string[]): Promise<{
  total: number;
  synced: number;
  failed: number;
...
export interface DeleteAssetsResponse {
  ...
  summary: {
    ...
    failed: number;
    localDeleted: number;
    expiredInGeminusPlus: number;
  };
}

/**
 * Delete one or more assets. Local assets are deleted directly,
 * synced assets are expired in Geminus Plus then removed locally.
...
export async function deleteAssets(
  ...
  const { data, error } = await supabase.functions.invoke("geminus-plus-delete", {
    body: {
      fmGuids,
      force: options?.force,
...
/**
 * Push all local-only assets to Geminus Plus via the sync edge function.
...
/**
 * Get count of local-only assets (not yet synced to Geminus Plus).
 */
 ...
 * Synced assets are expired in Geminus Plus before deletion.
 */
