/**
 * FmaV2ObjectGrid — FM Access object grid with column filter.
 */
...
function getNodeGuid(n: FmAccessNode): string {
  return n.systemGuid || n.guid || String(n.objectId ?? '');
}

function getNodeLabel(n: FmAccessNode): string {
  return n.objectName || n.name || String(n.objectId ?? '–');
}

function getMeta(n: FmAccessNode): Record<string, any> {
  return (n as any).metadata ?? n.properties ?? {};
}

// Pick most-populated non-GUID metadata columns, deduplicated
function pickColumns(objects: FmAccessNode[], max = 6): string[] {
