...
/**
 * Geminus Plus 3D Viewer Component
 * 
 * Integrates with the Geminus Plus 3D Viewer package to display BIM models.
 * Based on Geminus Plus external_viewer.html implementation pattern.
 */
const GeminusPlusViewer: React.FC<GeminusPlusViewerProps> = ({ 
  fmGuid, 
  ...
  // Deferred loading state (matching Geminus Plus pattern exactly)
  ...
  // to this single building. Without it, Geminus Plus returns the full complex (all buildings).
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      // Step 0: Activate the spaces layer in Geminus Plus BEFORE coloring
      setShowSpaces(true);
      try {
  ...
  const handleAnnotationsChange = useCallback((show: boolean) => {
    ...
    // Trigger Geminus Plus built-in annotation visibility update
    try {
  ...
  // Do display FMGUID (from Geminus Plus pattern - INTERNALS section)
  ...
  // Process deferred calls (from Geminus Plus pattern)
  ...
  const handleAllModelsLoaded = useCallback(() => {
    ...
    try {
      ...
      const openGeminusMenuAt = (clientX: number, clientY: number) => {
        ...
        // Remove any Geminus Plus menu that may be created async
      ...
      const apvDiv = document.getElementById('GeminusPlusViewer');
      ...
      if (!(window as any).__dxContextMenuObserver) {
        const observer = new MutationObserver((mutations) => {
          const hasGeminusPlusViewer = !!document.getElementById('GeminusPlusViewer');
          if (!hasGeminusPlusViewer) return;
  ...
  const setupCacheInterceptor = useCallback(() => {
    ...
    window.fetch = async (input: RequestInfo | URL, init?: RequestInit): Promise<Response> => {
      ...
      // Check if this is an XKT model request (only Geminus Plus API URLs, not Supabase/Storage)
      ...
      // The Geminus Plus SDK calls GetXktData WITHOUT bimobjectid → Geminus Plus returns geometry
      ...
      // Fetch from Geminus Plus API (using effectiveInput which has bimobjectid injected for GetXktData)
  ...
  const initializeViewer = useCallback(async () => {
    ...
    // CRITICAL FIX: Destroy and recreate #GeminusPlusViewer div to give Geminus Plus a fresh DOM anchor.
    // This prevents 'nextSibling' null errors when the old Vue runtime hasn't fully detached yet.
    const container = viewerContainerRef.current;
    container.innerHTML = '';
    const freshDiv = document.createElement('div');
    freshDiv.id = 'GeminusPlusViewer';
    // CRITICAL: CSS selector #GeminusPlusViewer.geminus-plus-hide-builtin-toolbar requires BOTH id AND class on the SAME element.
    // DX-classes must also be here so Geminus Plus Vue runtime finds correct device/theme context.
    freshDiv.className = [
      isMobile ? 'dx-device-mobile' : 'dx-device-desktop',
      'dx-device-generic', 'dx-theme-material', 'dx-theme-material-typography',
      'geminus-plus-hide-builtin-toolbar'
    ].join(' ');
    ...
    const timeoutId = setTimeout(() => {
      ...
      setState(prev => {
        if (prev.isLoading && !prev.isInitialized) {
          console.error(`[GeminusPlusViewer] Initialization timed out after ${INIT_TIMEOUT_MS}ms`);
          setInitStep('error');
          setShowError(true);
          return {
            ...prev,
    ...
    try {
      ...
      // Read actual token expiry from JWT payload (Geminus Plus staging tokens expire in 5 min!)
      ...
      if (!accessToken) {
        ...
        const devFetch = async (action: string): Promise<any> => {
          ...
          try {
            const r = await fetch('/__dev/geminus-plus-token', {
              method: 'POST',
        ...
        if (!tokenPayload?.accessToken) {
          const { data: efData, error: tokenError } = await supabase.functions.invoke('geminus-plus-query', {
        ...
        if (!accessToken) {
          throw new Error(
            import.meta.env.DEV
              ? 'Geminus Plus access token missing. Check GEMINUS_PLUS_* vars in .env and restart the dev server.'
              : 'Geminus Plus access token is missing. Check your API settings.'
          );
        }
      ...
      if (!assetplusviewer) {
        ...
        await new Promise<void>((resolve, reject) => {
          ...
          script.onerror = () => reject(new Error('Failed to load Geminus Plus 3D Viewer script'));
      ...
      if (!baseUrl) {
        ...
        if (!configPayload?.apiUrl) {
          const { data: efConfig } = await supabase.functions.invoke('geminus-plus-query', {
      ...
      // The Geminus Plus SDK calls GetXktData without bimobjectid → returns full-complex geometry.
      ...
      try {
        ...
        if (resolvedGuid) {
          ...
          // ── Source 2: Geminus Plus GetAllRelatedModels API ──────────────────────────────
          ...
          // Zero-API-call fallback — populated by the sync service from Geminus Plus storeys.
      ...
      const viewer = await assetplusviewer(
        ...
        async () => {
          ...
          try {
            ...
            if (import.meta.env.DEV) {
              try {
                const devRes = await fetch('/__dev/geminus-plus-token', {
                  method: 'POST',
                  headers: { 'Content-Type': 'application/json' },
            ...
            // Fallback: edge function
            if (!freshToken) {
              const { data } = await supabase.functions.invoke('geminus-plus-query', {
                body: { action: 'getToken' }
              });
        ...
        // externalCustomObjectContextMenuItems — empty array disables Geminus Plus built-in menu
        [],
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      // The 'e.nextSibling' error occurs when Geminus Plus tries to manipulate DOM
  ...
  // add it to the whitelist and re-trigger the Geminus Plus model load so the interceptor allows it through.
  useEffect(() => {
    const handleModelLoadRequested = (detail: ModelLoadRequestedDetail) => {
      ...
      // Re-trigger model load via Geminus Plus viewer API
      const viewer = viewerInstanceRef.current;
      const resolvedGuid = buildingFmGuid;
  ...
  // Viewer uses built-in Geminus Plus controls - no custom handlers needed
  ...
  return (
    <div className={isFullscreen ? "fixed inset-0 z-50 bg-background" : "h-full flex flex-col"}>
      {/* Viewer area with dx-viewport wrapper (CRITICAL for Geminus Plus popups) */}
      <div className={isFullscreen ? "h-full" : "flex-1 min-h-0"}>
        {/* dx-viewport wrapper - required by Geminus Plus for ObjectDetails popup constraint */}
        <div 
          ref={viewportWrapperRef}
          className="dx-viewport relative w-full h-full"
          style={{ margin: 0, display: 'flex', flexDirection: 'column' }}
        >
          {/* GeminusPlusViewer container - no id here; freshDiv inside gets id="GeminusPlusViewer" */}
          <div 
...
export default GeminusPlusViewer;
