/**
 * GeminusBase2DPanel — Embeds Geminus Base (HDC) web client in an iframe
...
const GeminusBase2DPanel: React.FC<GeminusBase2DPanelProps> = ({
  buildingFmGuid,
  floorId,
  floorName,
  ...
  const phaseLabels: Record<Phase, string> = {
    ...
    'loading-iframe': 'Loading Geminus Base...',
    'ready': '',
    'error': '',
  };
  ...
  useEffect(() => {
    ...
    async function fetchEmbedConfig() {
      ...
      try {
        ...
        if (!data?.success) {
          setError(data?.error || 'Geminus Base is not configured');
          setPhase('error');
          return;
        }
  ...
  }, [buildingFmGuid, floorId, floorName, geminusBaseBuildingGuid, buildingName, retryCount, noFloorSelected]);
  ...
  const handleMessage = useCallback((event: MessageEvent) => {
    ...
    if (event.data?.objectId || event.data?.guid || event.data?.fmGuid) {
      ...
      window.dispatchEvent(new CustomEvent('GEMINUS_BASE_CONTEXT_CHANGED', {
        detail: {
...
export default GeminusBase2DPanel;
