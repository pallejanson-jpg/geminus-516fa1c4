...
interface GeminusPremiumDashboardViewProps {
  onClose: () => void;
}
...
// ── Main component ──
const GeminusPremiumDashboardView: React.FC<GeminusPremiumDashboardViewProps> = ({ onClose }) => {
  const { geminusPremiumDashboardContext } = useContext(AppContext);
  const [iframeLoading, setIframeLoading] = useState(true);
  const [activeTab, setActiveTab] = useState('sensor');

  const facilityFmGuid = geminusPremiumDashboardContext?.facilityFmGuid ?? null;
  const facilityName = geminusPremiumDashboardContext?.facilityName || 'IoT Dashboard';

  const { data, isLoading, isLive, error } = useGeminusPremiumData(facilityFmGuid);

  // Prefer live dashboard URL from hook, fallback to context
  const dashboardUrl = data?.dashboardUrl || geminusPremiumDashboardContext?.dashboardUrl || '';
  ...
  const handleRefreshIframe = () => {
    ...
    const iframe = document.getElementById('geminus-premium-iframe') as HTMLIFrameElement;
    if (iframe) iframe.src = iframe.src;
  };

  if (!facilityFmGuid && !geminusPremiumDashboardContext?.dashboardUrl) {
    return (
      <div className="h-full flex flex-col items-center justify-center bg-background p-8">
        ...
        <p className="text-muted-foreground text-center max-w-md mb-6">
          No sensor dashboard configured for this object. Verify that the sensorDashboard attribute is set in Geminus Plus, or that the object is linked to Geminus Premium via FM GUID.
        </p>
      ...
      <div className={cn(
        ...
        <div className="flex items-center gap-3 min-w-0">
          ...
          <div className="min-w-0">
            ...
            <p className="text-xs text-muted-foreground">Geminus Premium IoT</p>
          </div>
        </div>
        ...
        <TabsContent value="sensor" className="flex-1 overflow-y-auto px-4 pb-4 mt-3 space-y-4">
          ...
          {!isLoading && error && (
            <div className="flex items-center gap-2 text-xs text-muted-foreground rounded-md border border-border px-3 py-2 bg-muted/30">
              ...
              <span>No live connection to Geminus Premium.</span>
            </div>
          )}
          {!isLoading && isLive && (
            ...
            <div className="absolute inset-0">
              {iframeLoading && (
                <div className="absolute inset-0 flex flex-col items-center justify-center bg-background/80 backdrop-blur-sm z-10">
                  ...
                  <span className="text-sm text-muted-foreground">Loading Geminus Premium dashboard…</span>
                </div>
              )}
              ...
              <iframe
                id="geminus-premium-iframe"
                src={dashboardUrl}
                className="w-full h-full border-0"
                title="Geminus Premium Dashboard"
...
export default GeminusPremiumDashboardView;
