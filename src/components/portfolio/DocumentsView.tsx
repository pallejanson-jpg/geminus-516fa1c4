...
const DocumentsView: React.FC<DocumentsViewProps> = ({ facility, onClose }) => {
  ...
  const fetchDocuments = async () => {
    if (!buildingFmGuid) { setIsLoading(false); return; }
    setIsLoading(true);
    try {
      ...
      const folders = new Set(docs.map(d => d.metadata?.congeria_path || 'Documents'));
  ...
  useEffect(() => { fetchDocuments(); }, [buildingFmGuid]);
  ...
  const handleSync = async () => {
    ...
    try {
      ...
      if (data?.suggestion === 'manual_upload') {
        toast({ title: 'Auto-sync failed', description: data.error || 'Use manual upload instead.' });
      } else if (data?.syncedCount > 0) {
        toast({ title: 'Sync complete', description: `${data.syncedCount} documents synced.` });
      } else if (data?.documentsFound === 0) {
        toast({ title: 'No documents found', description: 'Try manual upload.' });
      }

      await fetchDocuments();
    } catch (error) {
      console.error('Sync failed:', error);
      toast({ title: 'Sync failed', description: 'An unexpected error occurred. Try manual upload.', variant: 'destructive' });
    } finally {
      setIsSyncing(false);
    }
  };
  ...
  const handleFileUpload = async (files: FileList | null) => {
      ...
      try {
        ...
        const { error: dbError } = await supabase
          ...
          .upsert({
            ...
            metadata: { congeria_path: 'Manual uploads', uploaded_by: 'user' },
    ...
    if (uploadedCount > 0) {
      toast({ title: 'Upload complete', description: `${uploadedCount} documents uploaded${failedCount > 0 ? `, ${failedCount} failed` : ''}.` });
      await fetchDocuments();
    } else {
      toast({ title: 'Upload failed', description: 'No documents could be uploaded.', variant: 'destructive' });
    }
  };
  ...
  const groupedDocuments: GroupedDocuments[] = React.useMemo(() => {
    ...
    filteredDocs.forEach(doc => {
      const folder = doc.metadata?.congeria_path || 'Documents';
        ...
        {isLoading ? (
          ...
          <div className="space-y-2">
            {groupedDocuments.map(({ folder, documents }) => (
              <Collapsible key={folder} open={openFolders.has(folder)} onOpenChange={() => toggleFolder(folder)}>
                ...
                <CollapsibleContent className="ml-6 space-y-1 mt-1">
                  {documents.map((doc) => (
                    <div key={doc.id} className="flex items-center gap-3 p-2 hover:bg-muted rounded-lg group">
                      ...
                      <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                        ...
                        {doc.source_url && (
                          <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => window.open(doc.source_url!, '_blank')} title="Open original">
