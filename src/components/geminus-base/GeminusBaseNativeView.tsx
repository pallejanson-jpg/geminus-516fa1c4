...
import { useGeminusBaseApi, GeminusBaseNode } from '@/hooks/useGeminusBaseApi';
import { useIsMobile } from '@/hooks/use-mobile';
import GeminusBaseTree from './GeminusBaseTree';
import GeminusBaseObjectPanel from './GeminusBaseObjectPanel';
import GeminusBaseSearch from './GeminusBaseSearch';
import GeminusBaseDocuments from './GeminusBaseDocuments';
import GeminusPluginMenu from '@/components/viewer/GeminusPluginMenu';

const GeminusBaseNativeView: React.FC = () => {
  const { selectedFacility, navigatorTreeData } = useContext(AppContext);
  const isMobile = useIsMobile();
  const { getHierarchy, loading } = useGeminusBaseApi();
  ...
  const buildingFmGuid = manualFmGuid || selectedFacility?.fmGuid || (selectedFacility as any)?.geminus_base_building_guid;
  const buildingName = buildingFmGuid ? (selectedFacility?.name || 'Building') : 'Geminus Base 2.0';
  ...
  const handleNodeSelect = (node: GeminusBaseNode, _path?: GeminusBaseNode[]) => {
  ...
  const handleTreeSelect = (node: GeminusBaseNode, _path: GeminusBaseNode[]) => handleNodeSelect(node);
        ...
        <Card className="w-full max-w-md">
          <CardContent className="p-6 space-y-4">
            <div className="flex items-center gap-2 mb-2">
              ...
              <Badge variant="outline" className="text-[10px] ml-auto">Geminus Base 2.0</Badge>
            </div>
            <p className="text-sm text-muted-foreground">
              Select a building to load the Geminus Base hierarchy.
            </p>
          ...
          <TabsContent value="tree" className="flex-1 mt-0">
            {selectedNode ? (
              <div className="flex flex-col h-full">
                ...
                <GeminusBaseObjectPanel selectedNode={selectedNode} onRefresh={handleRefresh} onCreateChild={handleCreateChild} />
              </div>
            ) : (
              <GeminusBaseTree rootNode={rootNode} loading={treeLoading} selectedGuid={null} onSelect={handleTreeSelect} />
            )}
          </TabsContent>
          <TabsContent value="search" className="flex-1 mt-0">
            <GeminusBaseSearch onSelect={handleSearchSelect} />
          </TabsContent>
          <TabsContent value="docs" className="flex-1 mt-0">
            <GeminusBaseDocuments buildingId={buildingObjectId} buildingName={buildingName} />
          </TabsContent>
        </Tabs>
      </div>
    );
...
export default GeminusBaseNativeView;
