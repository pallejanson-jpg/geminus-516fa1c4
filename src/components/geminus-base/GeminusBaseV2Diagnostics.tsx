/**
 * GeminusBaseV2Diagnostics — probes Geminus Base endpoints to verify connectivity and discover API responses.
...
const GeminusBaseV2Diagnostics: React.FC = () => {
  ...
  const runDiagnostics = async () => {
    ...
    setDone(true);
    setRunning(false);
    console.log('[GeminusBaseV2 Diagnostics]', JSON.stringify(all, null, 2));
  };

  const download = () => {
    const blob = new Blob([JSON.stringify(results, null, 2)], { type: 'application/json' });
    const a = Object.assign(document.createElement('a'), {
      href: URL.createObjectURL(blob), download: 'geminus-base-diagnostics-v2.json',
    });
    a.click();
  };
...
export default GeminusBaseV2Diagnostics;
