...
import { useGeminusBaseApi, GeminusBaseDrawing, GeminusBaseDocument } from '@/hooks/useGeminusBaseApi';

interface GeminusBaseDocumentsProps {
  buildingId: string | null;
  buildingName?: string;
}

const GeminusBaseDocuments: React.FC<GeminusBaseDocumentsProps> = ({ buildingId, buildingName }) => {
  const { getDrawings, getDocuments, getDrawingPdf, loading } = useGeminusBaseApi();
  const [drawings, setDrawings] = useState<GeminusBaseDrawing[]>([]);
  const [documents, setDocuments] = useState<GeminusBaseDocument[]>([]);
...
export default GeminusBaseDocuments;
