...
/**
 * Geminus Plus Create Edge Function
 * 
 * Creates objects in Geminus Plus using the AddObjectList endpoint.
 ...
 * Geminus Plus REQUIRES a Building GUID as parent for Instance objects (ObjectType 4).
...
function resolveParent(item: CreateAssetItem): { parentFmGuid: string; roomFmGuid: string | null } {
  ...
  if (item.parentSpaceFmGuid) {
    // Only room provided — cannot create in Geminus Plus, use room as fallback
...
serve(async (req) => {
  ...
  try {
    ...
    if (!apiUrl) {
      return new Response(
        JSON.stringify({ success: false, error: "Geminus Plus API URL not configured" }),
    ...
    console.error("geminus-plus-create error:", error);
    return new Response(
      JSON.stringify({
        success: false,
