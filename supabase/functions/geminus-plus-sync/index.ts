...
import { getGeminusPlusCredentials } from "../_shared/credentials.ts";
...
async function getAccessToken(): Promise<string> {
  const keycloakUrl = _creds.keycloakUrl || Deno.env.get("GEMINUS_PLUS_KEYCLOAK_URL");
  const clientId = _creds.clientId || Deno.env.get("GEMINUS_PLUS_CLIENT_ID");
  const clientSecret = _creds.clientSecret || Deno.env.get("GEMINUS_PLUS_CLIENT_SECRET");
  const username = _creds.username || Deno.env.get("GEMINUS_PLUS_USERNAME");
  const password = _creds.password || Deno.env.get("GEMINUS_PLUS_PASSWORD");
...
// Uses Geminus Plus OpenAPI endpoints:
...
async function discover3dModelsEndpoint(
  ...
  await supabase.from('geminus_plus_endpoint_cache').delete().eq('key', 'getmodels_url');
...
// Fetch objects from Geminus Plus API with adaptive retry for sort memory errors
...
async function fetchGeminusPlusObjects(
  accessToken: string, 
  filter: any[], 
  skip = 0, 
  take = 200,
  ...
  if (!apiUrl || !apiKey) {
    throw new Error("Missing Geminus Plus API configuration");
  }
  ...
  if (!response.ok) {
    ...
    throw new Error(`Geminus Plus API failed: ${response.status} - ${errorText}`);
  }
...
async function fetchWithAdaptiveRetry(
  ...
  for (const take of takeSizes) {
    try {
      ...
      const result = await fetchGeminusPlusObjects(accessToken, filter, skip, take, {
...
// Get total count from Geminus Plus
...
// Write geometry mapping rows for synced Geminus Plus objects
...
// Prefixes for non-Geminus Plus data sources that should be excluded from orphan cleanup
const NON_GEMINUS_PLUS_PREFIXES = ['acc-bim-', 'acc-'];

function isNonGeminusPlusGuid(fmGuid: string): boolean {
  return NON_GEMINUS_PLUS_PREFIXES.some(prefix => fmGuid.startsWith(prefix));
}

// Fetch all fm_guids from local DB, paginating past the 1000-row default limit
// excludeNonGeminusPlus: when true, filters out fm_guids with non-Geminus Plus prefixes (e.g. acc-bim-*)
...
async function fetchAllLocalFmGuids(
  ...
  excludeNonGeminusPlus: boolean = false
  ...
  if (excludeNonGeminusPlus) {
    return allItems.filter(item => !isNonGeminusPlusGuid(item.fm_guid));
  }
  return allItems;
}
  ...
  try {
    ...
    if (action === 'sync-structure') {
      ...
      while (cleanupHasMore) {
        ...
        const result = await fetchGeminusPlusObjects(accessToken, cleanupFilter, cleanupSkip, cleanupTake);
      ...
      while (currentBuildingIndex < totalBuildings && !interrupted) {
        ...
        if (!force && currentSkip === 0 && pageMode === 'skip') {
          ...
          if (localAssetCount && localAssetCount > 0 && lastSyncAt) {
            ...
            try {
              const incrResult = await fetchGeminusPlusObjects(accessToken, incrFilter, 0, 1, {
    ...
    if (action === 'sync-single-building') {
      ...
      let totalSynced = 0;
      let skip = 0;
      // Keep batches small to avoid Geminus Plus backend Mongo sort memory limits
      const take = 200;
      let hasMore = true;
        ...
        try {
          ...
          for (const rev of modelsToSync) {
            ...
            try {
              ...
              // ALWAYS prefer the per-model buildingBimObjectId so Geminus Plus scopes the
    ...
    if (action === 'check-delta') {
      ...
      // Get remote building GUIDs to identify which buildings are in Geminus Plus
      const remoteBuildingGuids = new Set<string>();
      let skip = 0;
      const take = 200;
      let hasMore = true;
      while (hasMore) {
        const result = await fetchGeminusPlusObjects(accessToken, [["objectType", "=", 1]], skip, take);
      ...
      // We want to count ALL local objects that belong to Geminus Plus buildings
      ...
      // Filter out objects belonging to buildings not in Geminus Plus (IFC-only buildings)
    ...
    if (action === 'sync-with-cleanup') {
      ...
      for (const objectType of [1, 2, 3, 4]) {
        ...
        while (hasMore) {
          const result = await fetchGeminusPlusObjects(accessToken, [["objectType", "=", objectType]], skip, take);
      ...
      // ── Step 2: Remove orphans (local objects not in remote, within Geminus Plus scope) ──
      ...
      // ── Step 3: Push local objects missing from Geminus Plus ──
      // Collect ALL local objects (both is_local true and false) in Geminus Plus building scope
      ...
      // Filter to objects in Geminus Plus buildings, not already in remote, exclude ACC-prefixed
      const missingObjects = allLocalObjects.filter(obj => {
        if (isNonGeminusPlusGuid(obj.fm_guid)) return false;
      ...
      console.log(`Found ${missingObjects.length} local objects to push to Geminus Plus`);
      ...
      for (const obj of missingObjects) {
        try {
          ...
          if (response.ok) {
            ...
            pushedCount++;
            console.log(`✅ Pushed ${obj.category} ${obj.fm_guid} to Geminus Plus`);
          } else {
            const errorText = await response.text();
            pushFailedCount++;
        ...
        for (const asset of batch) {
          try {
            ...
            if (response.ok) {
              ...
              pushed++;
              console.log(`Pushed ${asset.fm_guid} to Geminus Plus`);
            } else {
              failed++;
    ...
    // ============ PUSH MISSING TO ASSET+ (IFC/ACC/local objects not yet in Geminus Plus) ============
    if (action === 'push-missing-to-geminus-plus') {
      console.log('Starting push-missing-to-geminus-plus');
      const accessToken = await getAccessToken();
      const apiUrl = _creds.apiUrl || Deno.env.get("GEMINUS_PLUS_API_URL") || "";
      const apiKey = _creds.apiKey || Deno.env.get("GEMINUS_PLUS_API_KEY") || "";
      const baseUrl = apiUrl.replace(/\/+$/, "");
      ...
      // Step 2: Find local objects not in remote, scoped to Geminus Plus buildings
      // Include both is_local=true and is_local=false (IFC/ACC created)
      const categories = ['Building Storey', 'Space', 'Instance'];
      const allLocalObjects: any[] = [];
      
      // Only query objects belonging to known Geminus Plus buildings to avoid huge scans
      ...
      // Filter: only objects in Geminus Plus buildings, not already in remote, exclude ACC-prefixed
      const missingObjects = allLocalObjects.filter(obj => {
        if (isNonGeminusPlusGuid(obj.fm_guid)) return false;
        if (remoteFmGuids.has(obj.fm_guid)) return false;
        // Must belong to an Geminus Plus building
        if (obj.building_fm_guid && !remoteBuildingGuids.has(obj.building_fm_guid)) return false;
        return true;
      });

      console.log(`Found ${missingObjects.length} local objects missing from Geminus Plus`);

      if (missingObjects.length === 0) {
        return new Response(
          JSON.stringify({ success: true, pushed: 0, failed: 0, message: 'Alla objekt finns redan i Geminus Plus' }),
          { headers: { ...corsHeaders, "Content-Type": "application/json" } }
        );
      }

      // Step 3: Push to Geminus Plus using AddObjectList, sorted by hierarchy
      ...
      for (const obj of missingObjects) {
        try {
          ...
          if (response.ok) {
            ...
            pushed++;
            console.log(`✅ Pushed ${obj.category} ${obj.fm_guid} (${obj.name}) to Geminus Plus`);
          } else {
            const errorText = await response.text();
            failed++;
      ...
      return new Response(
        JSON.stringify({
          ...
          message: `Skapade ${pushed} objekt i Geminus Plus${failed > 0 ? `, ${failed} misslyckades` : ''}`,
        }),
        { headers: { ...corsHeaders, "Content-Type": "application/json" } }
      );
    }
    ...
    if (action === 'sync-systems') {
      ...
      console.log('Starting sync-systems: extracting system data from Geminus Plus properties');
      ...
      // System name property keys to look for in Geminus Plus attributes
      ...
      while (currentBuildingIndex < totalBuildings && !interrupted) {
        ...
        for (const inst of instances) {
          ...
          // Also check nested property sets (some Geminus Plus data has properties inside sub-objects)
        ...
        for (const inst of instances) {
          extIdBatches.push({
            ...
            source: 'geminus_plus',
            external_id: inst.fm_guid, // Geminus Plus uses fmGuid as its own external ID
    ...
    console.error("geminus-plus-sync error:", error);
    return new Response(
