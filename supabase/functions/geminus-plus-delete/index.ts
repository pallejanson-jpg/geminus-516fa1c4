...
/** Batch expire synced fm_guids in Geminus Plus (max 50 per call) */
async function batchExpireInGeminusPlus(
  ...
  // If Geminus Plus credentials aren't configured, skip expiry gracefully
  if (!apiUrl || !apiKey) {
    console.log("Geminus Plus API not configured — skipping ExpireObject, treating all as local-only deletes");
  ...
  try {
    ...
    console.warn("Keycloak auth failed — skipping Geminus Plus expiry:", authErr);
...
async function handleDeleteAssets(body: any, supabase: any) {
  ...
  if (syncedFmGuids.length > 0) {
    ...
    const { expired, failed } = await batchExpireInGeminusPlus(syncedFmGuids, date);
    if (expired.length > 0) {
...
async function handleDeleteBuilding(body: any, supabase: any) {
  ...
  // 2. Expire synced assets in Geminus Plus
  let expiredCount = 0;
  let expireErrors: any[] = [];
  if (syncedFmGuids.length > 0) {
    addLog(`Expiring ${syncedFmGuids.length} synced objects in Geminus Plus...`);
    const { expired, failed } = await batchExpireInGeminusPlus(syncedFmGuids, new Date().toISOString());
  ...
  const cleanupTables = [
    ...
    { table: "geminus_base_drawings", column: "building_fm_guid" },
    { table: "geminus_base_documents", column: "building_fm_guid" },
    { table: "geminus_base_dou", column: "building_fm_guid" },
  ...
  const summary = {
    assetsDeleted,
    expiredInGeminusPlus: expiredCount,
...
serve(async (req) => {
  ...
  try {
    ...
    console.error("geminus-plus-delete error:", error);
    return new Response(
