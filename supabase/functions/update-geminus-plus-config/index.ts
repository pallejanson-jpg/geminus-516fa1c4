...
serve(async (req) => {
  ...
  // Verify authentication - config operations require admin
  const auth = await verifyAuth(req);
  if (!auth.authenticated) {
    return unauthorizedResponse(auth.error);
  }
  if (!auth.isAdmin) {
    return forbiddenResponse("Only admins can manage API configuration");
  }
  ...
  try {
    ...
    if (action === "test-connection") {
      ...
      const audience = Deno.env.get("GEMINUS_PLUS_AUDIENCE") || "asset-api";

      if (!keycloakUrl || !clientId) {
        return new Response(
          JSON.stringify({
            success: false,
            error: "Missing configuration. At minimum GEMINUS_PLUS_KEYCLOAK_URL and GEMINUS_PLUS_CLIENT_ID are required.",
            configured: {
      ...
      return new Response(
        JSON.stringify({
          ...
          config: {
            ...
            hasClientSecret: !!Deno.env.get("GEMINUS_PLUS_CLIENT_SECRET"),
            hasPassword: !!Deno.env.get("GEMINUS_PLUS_PASSWORD"),
            hasApiKey: !!Deno.env.get("GEMINUS_PLUS_API_KEY"),
          },
        }),
        { status: 200, headers: { ...corsHeaders, "Content-Type": "application/json" } }
      );
