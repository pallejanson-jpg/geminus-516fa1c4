...
async function addObjectToGeminusPlus(
  apiUrl: string,
  accessToken: string,
  payload: any
): Promise<any> {
  ...
  if (!response.ok) {
    let errorMsg = `Geminus Plus API error: ${response.status}`;
    try {
...
serve(async (req) => {
  ...
  try {
    ...
    const complexResult = await addObjectToGeminusPlus(apiUrl, accessToken, complexPayload);
    ...
    const buildingResult = await addObjectToGeminusPlus(apiUrl, accessToken, buildingPayload);
    ...
    const modelResult = await addObjectToGeminusPlus(apiUrl, accessToken, modelPayload);
    ...
    return new Response(
      JSON.stringify({
        ...
        modelFmGuid,
        modelName,
        message: `Fastighet "${body.complexName}", byggnad "${body.buildingName}" och modell "${modelName}" skapade i Geminus Plus`,
      }),
      { status: 200, headers: { ...corsHeaders, "Content-Type": "application/json" } }
    );
  } catch (error) {
    console.error("geminus-plus-create-building error:", error);
    return new Response(
      JSON.stringify({
        success: false,
