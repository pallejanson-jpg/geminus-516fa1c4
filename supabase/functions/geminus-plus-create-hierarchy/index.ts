...
/**
 * Geminus Plus Create Hierarchy Edge Function
 *
 * Creates Levels (ObjectType 2) and Spaces (ObjectType 3) in Geminus Plus
...
async function callGeminusPlusApi(apiUrl: string, accessToken: string, endpoint: string, payload: any): Promise<any> {
  ...
  if (!response.ok) {
    throw new Error(`Geminus Plus ${endpoint} error ${response.status}: ${text.slice(0, 300)}`);
  }
...
serve(async (req) => {
  ...
  try {
    ...
    if (body.levels?.length) {
      ...
      await callGeminusPlusApi(apiUrl, accessToken, "AddObjectList", {
    ...
    if (body.spaces?.length) {
      ...
      if (spacesWithLevel.length) {
        ...
        await callGeminusPlusApi(apiUrl, accessToken, "UpsertRelationships", {
    ...
    console.error("geminus-plus-create-hierarchy error:", error);
    return new Response(
      JSON.stringify({
        success: false,
