...
serve(async (req) => {
  ...
  try {
    ...
    // Helper: call geminus-base-query edge function internally
    async function callGeminusBase(fmAction: string, params: Record<string, unknown> = {}) {
      const resp = await fetch(`${supabaseUrl}/functions/v1/geminus-base-query`, {
        method: "POST",
        headers: {
    ...
    // Get all buildings with geminus_base_building_guid
    async function getBuildingsToSync(): Promise<{ fm_guid: string; geminus_base_building_guid: string }[]> {
      if (building_fm_guid) {
    ...
    switch (action) {
      case "sync-drawings": {
        await updateSyncState("geminus_base_drawings", "running");
        ...
        for (const b of buildings) {
          try {
            ...
            const treeResult = await callGeminusBase("get-perspective-tree", {
              guid: b.geminus_base_building_guid,
              perspectiveId: "8",
            });
            ...
            console.log(`Geminus Base sync: Found ${drawings.length} drawings for building ${b.fm_guid}`);

            for (const d of drawings) {
              const drawingId = String(d.objectId || d.ObjectId || d.id || "");
              if (!drawingId) continue;

              await supabase.from("geminus_base_drawings").upsert(
                {
        ...
        await updateSyncState("geminus_base_drawings", "completed", totalSynced);
        return new Response(
      ...
      case "sync-documents": {
        await updateSyncState("geminus_base_documents", "running");
        const buildings = await getBuildingsToSync();
        let totalSynced = 0;
        ...
        for (const b of buildings) {
          try {
            ...
            console.log(`Geminus Base sync: Found ${allNodes.length} document-candidate nodes for building ${b.fm_guid}`);

            for (const doc of allNodes) {
              const documentId = String(doc.objectId || doc.ObjectId || doc.id || "");
              if (!documentId) continue;

              await supabase.from("geminus_base_documents").upsert(
                {
                  building_fm_guid: b.fm_guid,
                ...
                await supabase.from("document_chunks").upsert(
                  {
                    ...
                    building_fm_guid: b.fm_guid,
                    file_name: node.objectName || node.ObjectName || "Geminus Base dokument",
                    content,
                    chunk_index: 0,
                    metadata: { system: "geminus_base", type: "document", classId: node.classId || node.ClassId },
                  },
                ...
                await supabase.from("document_chunks").upsert(
                  {
                    ...
                    building_fm_guid: b.fm_guid,
                    file_name: d.objectName || d.ObjectName || "Geminus Base ritning",
                    content,
                    chunk_index: 0,
                    metadata: { system: "geminus_base", type: "drawing", floor: d._parentFloorName },
                  },
                  { onConflict: "source_type,source_id,chunk_index", ignoreDuplicates: false }
                );
              }
                ...
                await supabase.from("document_chunks").upsert(
                  {
                    ...
                    building_fm_guid: b.fm_guid,
                    file_name: room.objectName || room.ObjectName || "Geminus Base rum",
                    content,
                    chunk_index: 0,
                    metadata: { system: "geminus_base", type: "room", floor: room._parentFloorName },
                  },
                  { onConflict: "source_type,source_id,chunk_index", ignoreDuplicates: false }
                );
              }
        ...
        await updateSyncState("geminus_base_documents", "completed", totalSynced);
        return new Response(
          JSON.stringify({ success: true, synced: totalSynced, buildings: buildings.length }),
          { headers: { ...corsHeaders, "Content-Type": "application/json" } }
        );
      ...
      case "sync-dou": {
        await updateSyncState("geminus_base_dou", "running");
        const buildings = await getBuildingsToSync();
        let totalSynced = 0;
        ...
        for (const b of buildings) {
          try {
            ...
            const hierResult = await callGeminusBase("get-perspective-tree", {
              guid: b.geminus_base_building_guid,
              perspectiveId: "8",
            });
            ...
            console.log(`Geminus Base sync: Found ${guids.length} GUIDs for DoU lookup (building ${b.fm_guid}), checking first 100`);

            // For each object, try to get DoU via proxy
            for (const guid of guids.slice(0, 100)) {
              try {
                const douResult = await callGeminusBase("proxy", {
                ...
                if (douResult?.success && douResult.data) {
                  ...
                  for (const item of items) {
                    ...
                    await supabase.from("geminus_base_dou").insert({
                      object_fm_guid: guid,
                      building_fm_guid: b.fm_guid,
                      ...
                      await supabase.from("document_chunks").upsert(
                        {
                          ...
                          chunk_index: 0,
                          metadata: { system: "geminus_base", type: "dou", object_guid: guid },
                        },
                        { onConflict: "source_type,source_id,chunk_index", ignoreDuplicates: false }
                      );
                    }
        ...
        await updateSyncState("geminus_base_dou", "completed", totalSynced);
        return new Response(
          JSON.stringify({ success: true, synced: totalSynced, buildings: buildings.length }),
          { headers: { ...corsHeaders, "Content-Type": "application/json" } }
        );
      ...
      case "sync-all": {
        ...
        for (const syncAction of ["sync-drawings", "sync-documents", "sync-dou"]) {
          try {
            const resp = await fetch(`${supabaseUrl}/functions/v1/geminus-base-sync`, {
              method: "POST",
              headers: {
                "Content-Type": "application/json",
                Authorization: `Bearer ${serviceKey}`,
      ...
      case "get-status": {
        const { data } = await supabase
          ...
          .select("*")
          .in("sync_type", ["geminus_base_drawings", "geminus_base_documents", "geminus_base_dou"]);

        // Get counts
        const [drawings, documents, dou] = await Promise.all([
          supabase.from("geminus_base_drawings").select("id", { count: "exact", head: true }),
          supabase.from("geminus_base_documents").select("id", { count: "exact", head: true }),
          supabase.from("geminus_base_dou").select("id", { count: "exact", head: true }),
        ]);
    ...
    console.error("geminus-base-sync error:", err);
    return new Response(
