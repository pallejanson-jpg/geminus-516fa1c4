...
interface GeminusPremiumRequest {
...
async function getKeycloakToken(
  ...
  if (clientSecret) {
    console.log('[Geminus Premium] Trying Keycloak client_credentials (service account)');
    ...
    if (res.ok) {
      ...
      console.log('[Geminus Premium] Keycloak client_credentials OK');
      return { token: data.access_token, type: 'Bearer' };
    }
    const errText = await res.text();
    console.warn('[Geminus Premium] client_credentials failed:', res.status, errText, '— falling back to password grant');
  }

  // Variant B: password grant with AD account
  if (username && password) {
    console.log('[Geminus Premium] Trying Keycloak password grant (AD account)');
    const params = new URLSearchParams({
      grant_type: 'password',
      client_id: clientId,
      username,
    ...
    const data = await res.json();
    console.log('[Geminus Premium] Keycloak password grant OK');
    return { token: data.access_token, type: 'Bearer' };
  }
...
async function getDjangoToken(apiUrl: string, email: string, password: string): Promise<string> {
  ...
  console.log('[Geminus Premium] Authenticating via Django token auth:', tokenUrl);
  ...
  for (let retry = 0; retry <= maxRetries; retry++) {
    if (retry > 0) {
      console.log(`[Geminus Premium] Auth retry ${retry}/${maxRetries} after ${delay}ms`);
    ...
    for (const attempt of attempts) {
      ...
      if (response.status === 429) {
        ...
        console.warn('[Geminus Premium] Rate limited (429) on auth attempt:', attempt.label, text);
        got429 = true;
        break;
      }
      ...
      if (!response.ok) {
        ...
        lastText = text;
        console.error('[Geminus Premium] Auth failed:', response.status, text);
        continue;
      }

      const data = await response.json();
      if (!data.token) {
        throw new Error('No token received from Geminus Premium');
      }

      console.log('[Geminus Premium] Django token auth OK (payload:', attempt.label, ')');
      return data.token;
    }
...
async function getTokenWithType(
  ...
  if (cachedToken && Date.now() < cachedToken.expiresAt) {
    console.log('[Geminus Premium] Using cached token (type:', cachedToken.type, ')');
    return { token: cachedToken.token, type: cachedToken.type };
  }

  const keycloakUrl = Deno.env.get('GEMINUS_PREMIUM_KEYCLOAK_URL');
  const clientId = Deno.env.get('GEMINUS_PREMIUM_CLIENT_ID');
  const clientSecret = Deno.env.get('GEMINUS_PREMIUM_CLIENT_SECRET');
...
async function geminusPremiumFetchWithRetry(
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    console.log(`[Geminus Premium] Fetching: ${url} (attempt ${attempt + 1}, auth: ${tokenType})`);
    ...
    if (!response.ok) {
      const text = await response.text();
      console.error('[Geminus Premium] Request failed:', response.status, text);
      throw new Error(`Geminus Premium API error: ${response.status} - ${text}`);
    }
...
// Legacy wrapper
async function geminusPremiumFetch(apiUrl: string, endpoint: string, token: string, tokenType: 'JWT' | 'Bearer' = 'JWT') {
  return geminusPremiumFetchWithRetry(apiUrl, endpoint, token, tokenType);
}
...
function buildDashboardUrl(apiUrl: string, type: 'machine' | 'site' | 'line', pk: number): string {
  ...
  // Many Geminus Premium instances use the same domain without the api. prefix,
...
serve(async (req) => {
  ...
  try {
    const { action, fmGuid, siteCode, indiceId, workspaceKey, query, days, contextLevel, question, conversationHistory, buildingFmGuid } = await req.json() as GeminusPremiumRequest & { buildingFmGuid?: string };

    // Resolve per-building credentials
    const supabaseUrl = Deno.env.get('SUPABASE_URL')!;
    const supabaseKey = Deno.env.get('SUPABASE_SERVICE_ROLE_KEY')!;
    const sbClient = createClient(supabaseUrl, supabaseKey);
    const slCreds = await getGeminusPremiumCredentials(sbClient, buildingFmGuid || fmGuid);
    ...
    if (!apiUrl || !email || !password) {
      return new Response(
        JSON.stringify({
          success: false,
          error: 'Geminus Premium credentials not configured',
          message: 'Please configure Geminus Premium credentials in Lovable Cloud or in building settings.'
        }),
        { headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      );
    }
    ...
    const authedActions = new Set<GeminusPremiumRequest['action']>([
      'get-equipment',
      'get-site-equipment',
      'get-sites',
      'get-lines',
      ...
      try {
        ...
        tokenType = result.type;
          ...
          return jsonResponse({
            ...
            message: 'Could not connect to Geminus Premium. Check credentials.',
          });
      }
    }
    ...
    switch (action) {
      case 'test-connection': {
        try {
          const authMode = Deno.env.get('GEMINUS_PREMIUM_KEYCLOAK_URL') ? 'Keycloak' : 'Django token';
          const { token: t, type } = await getTokenWithType(cleanApiUrl, email, password);
          const sites = await geminusPremiumFetch(cleanApiUrl, '/api/sites', t, type);
          return jsonResponse({
            success: true,
      ...
      case 'get-equipment': {
        ...
        const machines = await geminusPremiumFetch(cleanApiUrl, `/api/machines?code=${encodeURIComponent(fmGuid)}`, authToken, tokenType);
      ...
      case 'get-site-equipment': {
        ...
        const authToken = token as string;
        const machines = await geminusPremiumFetch(cleanApiUrl, `/api/machines?site=${encodeURIComponent(siteCode)}`, authToken, tokenType);
        return jsonResponse({ success: true, data: machines });
      }

      case 'get-sites': {
        const authToken = token as string;
        const sites = await geminusPremiumFetch(cleanApiUrl, '/api/sites', authToken, tokenType);
        return jsonResponse({ success: true, data: sites });
      }

      case 'get-lines': {
        const authToken = token as string;
        const lines = await geminusPremiumFetch(cleanApiUrl, '/api/lines', authToken, tokenType);
        return jsonResponse({ success: true, data: lines });
      }

      case 'get-machines': {
        const authToken = token as string;
        const machines = await geminusPremiumFetch(cleanApiUrl, '/api/machines', authToken, tokenType);
        return jsonResponse({ success: true, data: machines });
      }
      ...
      case 'get-dashboard-url': {
        ...
        try {
          ...
          console.log('[Geminus Premium] No machine found for fmGuid:', fmGuid);
        }

        try {
          const sites = await geminusPremiumFetch(cleanApiUrl, `/api/sites?code=${encodeURIComponent(fmGuid)}`, authToken, tokenType);
          ...
          console.log('[Geminus Premium] No site found for fmGuid:', fmGuid);
        }

        try {
          const lines = await geminusPremiumFetch(cleanApiUrl, `/api/lines?code=${encodeURIComponent(fmGuid)}`, authToken, tokenType);
          ...
          console.log('[Geminus Premium] No line found for fmGuid:', fmGuid);
        }
      ...
      case 'get-indices': {
        const authToken = token as string;
        const indices = await geminusPremiumFetchWithRetry(cleanApiUrl, '/api/indices', authToken, tokenType);
      ...
      case 'get-properties': {
        ...
        const authToken = token as string;
        const properties = await geminusPremiumFetchWithRetry(cleanApiUrl, `/api/properties?indice=${indiceId}`, authToken, tokenType);
      ...
      case 'search-data': {
        ...
        const authToken = token as string;
        const results = await geminusPremiumFetchWithRetry(
          cleanApiUrl,
      ...
      case 'get-machine-data': {
        ...
        const machinesRaw = await geminusPremiumFetchWithRetry(cleanApiUrl, `/api/machines?code=${encodeURIComponent(fmGuid)}`, authToken, tokenType);
        ...
        if (Array.isArray(machine.indices) && machine.indices.length > 0) {
          try {
            ...
            const propsRaw = await geminusPremiumFetchWithRetry(cleanApiUrl, `/api/properties?indice=${indiceId}`, authToken, tokenType) as any;
            ...
            if (properties.length > 0) {
              ...
              if (Array.isArray(p0.natural_key) && p0.natural_key.length > 0) {
                ...
                console.log('[Geminus Premium] Workspace key from natural_key:', workspaceKeyDiscovered);
              } else if (p0.indice_workspace) {
                workspaceKeyDiscovered = p0.indice_workspace;
                console.log('[Geminus Premium] Workspace key from indice_workspace:', workspaceKeyDiscovered);
              }
            }
          } catch (e) {
            console.warn('[Geminus Premium] Could not fetch properties:', e);
          }
        }
        ...
        // Strategy A: ES data-workspaces/_search (works on some Geminus Premium instances)
        ...
        if (wsKey) {
          try {
            ...
            timeSeries = await geminusPremiumFetchWithRetry(
              cleanApiUrl,
              `/api/data-workspaces/${encodeURIComponent(wsKey)}/_search`,
              authToken, tokenType,
              { method: 'POST', body: esQuery }
            );
            console.log('[Geminus Premium] ES data-workspaces OK for key:', wsKey);
          } catch (e) {
            console.warn('[Geminus Premium] ES data-workspaces failed, trying /api/machines/{pk}/data/:', (e as Error).message);
          }
        }
          ...
          try {
            ...
            for (const indKey of indiceKeys) {
              try {
                ...
                const dataResult = await geminusPremiumFetchWithRetry(cleanApiUrl, dataUrl, authToken, tokenType) as any;
                if (dataResult && (Array.isArray(dataResult) ? dataResult.length > 0 : dataResult.results?.length > 0)) {
                  ...
                  console.log('[Geminus Premium] /api/machines/{pk}/data/ OK for indice:', indKey, 'rows:', machineDataResult.length);
                  break;
                }
              } catch (innerErr) {
                console.warn('[Geminus Premium] machines/data failed for indice', indKey, ':', (innerErr as Error).message);
              }
            }
          } catch (e) {
            console.warn('[Geminus Premium] Could not fetch /api/machines/{pk}/data/:', (e as Error).message);
          }
        }
          ...
          try {
            ...
            for (const indKey of indiceKeys) {
              try {
                ...
                const latestResult = await geminusPremiumFetchWithRetry(cleanApiUrl, latestUrl, authToken, tokenType) as any;
                if (latestResult && (Array.isArray(latestResult) ? latestResult.length > 0 : latestResult.results?.length > 0)) {
                  ...
                  console.log('[Geminus Premium] /api/machines/{pk}/data/?last=1 OK for indice:', indKey);
                  break;
                }
              } catch (innerErr) {
            ...
            console.warn('[Geminus Premium] Could not fetch latest values:', (e as Error).message);
          }
        }
      ...
      case 'get-machine-air-quality': {
        ...
        // Find machine
        const machinesRaw2 = await geminusPremiumFetchWithRetry(cleanApiUrl, `/api/machines?code=${encodeURIComponent(fmGuid)}`, authToken, tokenType);
        ...
        for (const indKey of indiceKeys2) {
          try {
            ...
            const result2 = await geminusPremiumFetchWithRetry(cleanApiUrl, url2, authToken, tokenType) as any;
            const arr = Array.isArray(result2) ? result2 : (result2?.results ?? []);
            if (arr.length > 0) allData[indKey] = arr;
          } catch (e) {
            console.warn('[Geminus Premium] air-quality data for indice', indKey, 'failed:', (e as Error).message);
          }
        }
        ...
        for (const indKey of indiceKeys2) {
          try {
            const propsRaw2 = await geminusPremiumFetchWithRetry(cleanApiUrl, `/api/properties?indice=${indKey}`, authToken, tokenType) as any;
      ...
      case 'get-building-sensor-data': {
        ...
        const sitesRaw = await geminusPremiumFetchWithRetry(cleanApiUrl, `/api/sites?code=${encodeURIComponent(fmGuid)}`, authToken, tokenType) as any;
        const sitesArr = Array.isArray(sitesRaw) ? sitesRaw : (sitesRaw?.results ?? []);

        if (sitesArr.length === 0) {
          return jsonResponse({ success: false, error: 'No Geminus Premium site found for this building fmGuid' });
        }
        const site = sitesArr[0] as any;

        // Fetch all machines for this site using site pk
        const machinesRaw = await geminusPremiumFetchWithRetry(cleanApiUrl, `/api/machines?site=${site.pk}`, authToken, tokenType) as any;
        ...
        if (!hasAnyLatest && machineSlim.length > 0) {
          console.log(`[Geminus Premium] All latest_values null, fetching detail for sample of ${Math.min(15, machineSlim.length)} machines...`);
          ...
          for (let i = 0; i < machineSlim.length && detailPromises.length < sampleSize; i += step) {
            ...
            detailPromises.push((async () => {
              try {
                const detail = await geminusPremiumFetchWithRetry(cleanApiUrl, `/api/machines/${m.pk}/`, authToken, tokenType) as any;
                if (detail?.latest_values) {
                  machineSlim[idx] = { ...machineSlim[idx], latest_values: detail.latest_values };
                  console.log(`[Geminus Premium] Got latest_values from detail for machine ${m.pk} (${m.name})`);
                }
              } catch (e) {
                console.warn(`[Geminus Premium] Detail fetch failed for machine ${m.pk}:`, (e as Error).message);
              }
            })());
          }
      ...
      case 'get-ilean-context': {
        ...
        if (level === 'building') {
          ...
          try {
            const sites = await geminusPremiumFetchWithRetry(cleanApiUrl, `/api/sites?code=${encodeURIComponent(fmGuid)}`, authToken, tokenType) as any;
            ...
            console.warn('[Geminus Premium] get-ilean-context building lookup failed:', e);
          }
        } else if (level === 'floor') {
          // Find line by code = fmGuid
          try {
            const lines = await geminusPremiumFetchWithRetry(cleanApiUrl, `/api/lines?code=${encodeURIComponent(fmGuid)}`, authToken, tokenType) as any;
            ...
            console.warn('[Geminus Premium] get-ilean-context floor lookup failed:', e);
          }
        } else {
          // Room: find machine by code = fmGuid
          try {
            const machines = await geminusPremiumFetchWithRetry(cleanApiUrl, `/api/machines?code=${encodeURIComponent(fmGuid)}`, authToken, tokenType) as any;
            ...
            console.warn('[Geminus Premium] get-ilean-context room lookup failed:', e);
          }
        }

        return jsonResponse({ success: false, error: `No Geminus Premium entity found for fmGuid at level: ${level}` });
      }
      ...
      // ── ilean-ask: document Q&A via Geminus Premium Ilean API or Lovable AI fallback ──
      case 'ilean-ask': {
        ...
        if (fmGuid) {
          try {
            if (level === 'room') {
              // Room → Machine in Geminus Premium
              const machines = await geminusPremiumFetchWithRetry(cleanApiUrl, `/api/machines?code=${encodeURIComponent(fmGuid)}`, authToken, tokenType) as any;
            ...
            console.warn('[Geminus Premium] Could not resolve entity for ilean-ask:', e);
          }
        }

        console.log(`[Geminus Premium] ilean-ask context: level=${level}, entityApiType=${entityApiType}, pk=${entityPk}, name=${entityName}`);

        // Try Geminus Premium Ilean API endpoints — use the resolved entity type and PK
        ...
        for (const endpoint of ileanEndpoints) {
          try {
            console.log('[Geminus Premium] Trying Ilean endpoint:', endpoint);
            ...
            if (resp.ok) {
              ...
              console.log('[Geminus Premium] Ilean API response OK from:', endpoint);
              const answer = data.answer || data.response || data.message || data.content || JSON.stringify(data);
              return jsonResponse({ success: true, data: { answer, source: 'geminus-premium-ilean', endpoint } });
            }
            ...
            console.warn('[Geminus Premium] Ilean endpoint returned', resp.status, ':', errText.slice(0, 200));
          } catch (e) {
            console.warn('[Geminus Premium] Ilean endpoint error:', endpoint, (e as Error).message);
          }
        }
...
Svara ALLTID på samma språk som användaren skriver.`;
        ...
        try {
          ...
          if (!aiResp.ok) {
            ...
            return jsonResponse({
              success: true,
              data: {
                answer: 'I apologize, but I\'m unable to process your question at the moment. Please try again later or check the Geminus Premium portal directly.',
                source: 'fallback-error',
              },
            });
          }
    ...
    console.error('[Geminus Premium] Error:', error);
