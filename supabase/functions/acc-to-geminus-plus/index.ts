...
/**
 * acc-to-geminus-plus: Syncs ACC-sourced objects from the local database to Geminus Plus API.
 * 
 * Flow:
 * 1. Fetch ACC objects from `assets` table (source = 'acc-bim')
 * 2. For each building: create Complex -> Building -> Levels -> Spaces -> Instances in Geminus Plus
 * 3. Store GUID mappings in `acc_geminus_plus_guid_map`
 * 4. Set relationships and properties via Geminus Plus API
 */
...
// ============ Geminus Plus API helpers ============

async function geminusPlusPost(endpoint: string, body: any, accessToken: string): Promise<any> {
  const apiUrl = Deno.env.get("GEMINUS_PLUS_API_URL")!.replace(/\/+$/, "");
  ...
  if (!res.ok) {
    console.error(`Geminus Plus ${endpoint} failed (${res.status}):`, text.substring(0, 500));
    throw new Error(`Geminus Plus ${endpoint}: ${res.status} - ${text.substring(0, 200)}`);
  }
...
async function getOrCreateGuid(
  ...
  const { data: existing } = await supabase
    .from("acc_geminus_plus_guid_map")
    .select("geminus_plus_fm_guid")
    .eq("acc_fm_guid", accFmGuid)
    .maybeSingle();

  if (existing?.geminus_plus_fm_guid) {
    return existing.geminus_plus_fm_guid;
  }
...
async function addObjectList(
  ...
  return await geminusPlusPost("AddObjectList", payload, accessToken);
}
...
async function upsertRelationships(
  ...
  return await geminusPlusPost("UpsertRelationships", payload, accessToken);
}
...
async function updateProperties(
  ...
  return await geminusPlusPost("UpdateBimObjectsPropertiesData", payload, accessToken);
}

// ============ Check if object exists in Geminus Plus ============

async function checkExistsInGeminusPlus(
  fmGuid: string,
  accessToken: string,
  apiKey: string
): Promise<boolean> {
  try {
    const result = await geminusPlusPost("PublishDataServiceGetMerged", {
      outputType: "raw",
      apiKey,
...
async function syncBuildingToGeminusPlus(
  supabase: any,
  ...
  try {
    ...
    // Check if complex exists in Geminus Plus
    const complexExists = await checkExistsInGeminusPlus(complexFmGuid, accessToken, apiKey);
    if (!complexExists) {
      try {
        await addObjectList([{
    ...
    // Step 2: Get/create Geminus Plus GUID for building
    const buildingApGuid = await getOrCreateGuid(supabase, buildingFmGuid, ObjectType.Building);

    // Step 3: Check if building already exists in Geminus Plus
    const buildingExists = await checkExistsInGeminusPlus(buildingApGuid, accessToken, apiKey);
    ...
    if (levels.length > 0) {
      ...
      for (const level of levels) {
        ...
        const exists = await checkExistsInGeminusPlus(apGuid, accessToken, apiKey);
        if (!exists) {
          levelItems.push({
    ...
    // Step 8: Update local assets with Geminus Plus GUID mappings
      ...
      if (apGuid) {
        ...
        await supabase
          ...
          .update({
            attributes: {
              ...attrs,
              geminus_plus_fm_guid: apGuid,
              synced_to_geminus_plus: true,
              synced_to_geminus_plus_at: new Date().toISOString(),
            },
          })
...
async function handleCheckStatus(supabase: any): Promise<Response> {
  // Count ACC objects not yet synced to Geminus Plus
  ...
  for (const b of (accBuildings || [])) {
    const { data: mapping } = await supabase
      .from("acc_geminus_plus_guid_map")
      .select("synced_at, geminus_plus_fm_guid")
    ...
    buildingStatuses.push({
      ...
      geminusPlusFmGuid: mapping?.geminus_plus_fm_guid || null,
  ...
  return new Response(JSON.stringify({
    ...
    syncedToGeminusPlus: syncedCount || 0,
...
async function handleSync(
  ...
  if (!apiUrl || !apiKey) {
    return new Response(JSON.stringify({ success: false, error: "Geminus Plus API not configured" }), {
  ...
  for (const building of buildings) {
    ...
    const result = await syncBuildingToGeminusPlus(
      supabase,
      building.fm_guid,
      accessToken,
      apiKey
...
serve(async (req) => {
  ...
  try {
    ...
    console.error("acc-to-geminus-plus error:", err);
    return new Response(JSON.stringify({
      success: false,
