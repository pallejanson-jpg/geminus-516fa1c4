...
async function getToken(config: GeminusBaseConfig): Promise<string> {
  ...
  if (!response.ok) {
    ...
    console.error('Geminus Base token error:', response.status, errorText);
...
/**
 * Get Geminus Base system version ID (required header for most API calls)
 */
async function getVersionId(config: GeminusBaseConfig, token: string): Promise<string> {
  if (versionIdCache && Date.now() - versionIdCache.fetchedAt < 5 * 60 * 1000) {
    return versionIdCache.versionId;
  }

  console.log('Geminus Base: Fetching version ID from', config.apiUrl);
  ...
  try {
    ...
    if (response.ok) {
      ...
      console.log('Geminus Base: systeminfo response:', JSON.stringify(data).substring(0, 500));
      ...
      if (versionId) {
        ...
        console.log('Geminus Base: Version ID obtained:', versionId);
        return String(versionId);
      }
    } else {
      const text = await response.text();
      console.log('Geminus Base: systeminfo returned', response.status, text.substring(0, 100));
    }
  } catch (e) {
    console.log('Geminus Base: systeminfo error:', e.message);
  }

  console.log('Geminus Base: Could not get versionId, proceeding without it');
  versionIdCache = { versionId: '', fetchedAt: Date.now() };
  return '';
}

/**
 * Make an authenticated Geminus Base API call.
 * FIX: Only set Content-Type: application/json for non-GET requests.
 */
async function geminusBaseFetch(
  config: GeminusBaseConfig,
  path: string,
  options: RequestInit = {}
): Promise<Response> {
  ...
  console.log('Geminus Base: Calling', method, url);
...
// ── IFC/Geminus Plus → Geminus Base target class mapping ────────────────────
...
/**
 * Resolve Geminus Base target class from IFC type and/or Geminus Plus category.
 * Geminus Base uses target class names (e.g. fmo_equip_door) to route objects
...
function resolveTargetClass(ifcType?: string, category?: string): TargetClassResult {
  ...
  // Building element proxy → electrical appliance (IFC) or appliance (Geminus Plus)
...
/**
 * Map Geminus/Geminus Plus property names to Geminus Base domain-specific field names
 * based on the resolved target class.
 */
function mapPropertiesToGeminusBase(targetClass: string, props: Record<string, any>): Record<string, any> {
...
/**
 * Resolve Geminus Base building GUID from perspective root if not provided.
...
async function resolveBuildingGuid(
  ...
  console.log('Geminus Base: Looking up building by name:', buildingName);
  try {
    const rootResp = await geminusBaseFetch(config, '/api/perspective/root/json/8');
    if (!rootResp.ok) {
      console.log('Geminus Base: Perspective root returned', rootResp.status);
      return null;
    }
    ...
    if (!match) {
      ...
      console.log('Geminus Base: No match for "' + buildingName + '", available:', JSON.stringify(names));
      return null;
    }

    const guid = match.systemGuid || match.objectGuid || match.ObjectGuid || match.guid || match.Guid;
    console.log('Geminus Base: Matched building:', match.objectName || match.ObjectName, '-> GUID:', guid);
      ...
      try {
        ...
        if (supabaseUrl && serviceRoleKey) {
          await fetch(`${supabaseUrl}/rest/v1/building_settings?fm_guid=eq.${encodeURIComponent(buildingId)}`, {
            ...
            body: JSON.stringify({ geminus_base_building_guid: guid }),
          });
        }
      } catch (e) {
        console.log('Geminus Base: Failed to cache GUID:', e.message);
      }
    }

    return guid;
  } catch (e) {
    console.log('Geminus Base: Building lookup failed:', e.message);
    return null;
  }
}
...
async function resolveDrawingObjectId(
  ...
  console.log('Geminus Base: Looking up drawing for guid', lookupGuid, 'floorName:', floorName || '(none)');

  const treeResp = await geminusBaseFetch(config, `/api/perspective/byguid/subtree/json/8/${encodeURIComponent(lookupGuid)}`);
  if (!treeResp.ok) {
    const errText = await treeResp.text();
    console.log('Geminus Base: perspective tree error', treeResp.status, errText.substring(0, 200));
    return null;
  }
  ...
  if (floorName && buildingGuid) {
    ...
    console.log('Geminus Base: Found', floorNodes.length, 'floor nodes (classId 105)');
    ...
    if (matchedFloor) {
      console.log('Geminus Base: Matched floor:', matchedFloor.objectName || matchedFloor.ObjectName);
      const drawing = findDrawingUnder(matchedFloor);
      if (drawing) {
        const id = drawing.objectId || drawing.ObjectId || drawing.id || drawing.Id || null;
        console.log('Geminus Base: Found drawing under floor, objectId:', id);
  ...
  if (drawingNode) {
    ...
    console.log('Geminus Base: Using fallback drawing, objectId:', id);
    return id ? String(id) : null;
  }

  console.log('Geminus Base: No drawing found in perspective tree');
  return null;
}
...
serve(async (req) => {
  ...
  try {
    ...
    try {
      ...
      console.log('Geminus Base: Could not load credentials from DB, using env vars:', e.message);
    }

    const config: GeminusBaseConfig = {
      tokenUrl: Deno.env.get('GEMINUS_BASE_TOKEN_URL') || 'https://auth.bim.cloud/auth/realms/swg_demo/protocol/openid-connect/token',
      clientId: Deno.env.get('GEMINUS_BASE_CLIENT_ID') || 'HDCAgent Basic',
      clientSecret: Deno.env.get('GEMINUS_BASE_CLIENT_SECRET'),
      apiUrl: (dbApiUrl || Deno.env.get('GEMINUS_BASE_API_URL') || '').replace(/\/+$/, ''),
      username: dbUsername || Deno.env.get('GEMINUS_BASE_USERNAME'),
    ...
    switch (action) {
      ...
      case 'get-embed-config': {
        const { buildingId, floorName, geminusBaseBuildingGuid, buildingName } = params;
        try {
          const token = await getToken(config);
          const versionId = await getVersionId(config, token);

          // Resolve building GUID
          const resolvedGuid = await resolveBuildingGuid(config, buildingName, buildingId, geminusBaseBuildingGuid);
      ...
      case 'get-viewer-url': {
        ...
        let { geminusBaseBuildingGuid } = params;
        try {
          const token = await getToken(config);
          const versionId = await getVersionId(config, token);
          ...
          // Resolve building GUID
          const resolvedGuid = await resolveBuildingGuid(config, buildingName, buildingId, geminusBaseBuildingGuid);
          geminusBaseBuildingGuid = resolvedGuid || geminusBaseBuildingGuid;

          // Resolve drawing objectId
          const drawingObjectId = await resolveDrawingObjectId(
            config,
            geminusBaseBuildingGuid || '',
            floorName,
            floorId,
            buildingId
          );
      ...
      case 'get-drawings': {
        ...
        const response = await geminusBaseFetch(config, `/api/drawings?buildingId=${encodeURIComponent(buildingId)}`);
        const text = await response.text();
        let data;
        try { data = JSON.parse(text); } catch { data = null; }
        return new Response(
          JSON.stringify({ success: response.ok, data: data || [], error: !response.ok ? `Geminus Base returned ${response.status}` : undefined }),
          { headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
        );
      }
      ...
      case 'get-documents': {
        ...
        const response = await geminusBaseFetch(config, `/api/documents?buildingId=${encodeURIComponent(buildingId)}`);
        const text = await response.text();
        let data;
        try { data = JSON.parse(text); } catch { data = null; }
        return new Response(
      ...
      case 'get-document': {
        ...
        const response = await geminusBaseFetch(config, `/api/documents/${encodeURIComponent(documentId)}`);
        const text = await response.text();
        let data;
        try { data = JSON.parse(text); } catch { data = null; }
        return new Response(
          JSON.stringify({ success: response.ok, data, error: !response.ok ? `Geminus Base returned ${response.status}` : undefined }),
          { headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
        );
      }
      ...
      case 'get-floors': {
        ...
        const response = await geminusBaseFetch(config, `/api/floors?buildingId=${encodeURIComponent(buildingFmGuid)}`);
        const text = await response.text();
        let data;
        try { data = JSON.parse(text); } catch { data = null; }
        return new Response(
      ...
      case 'get-object-by-guid': {
        ...
        const response = await geminusBaseFetch(config, `/api/object/byguid/json/${encodeURIComponent(guid)}`);
        const text = await response.text();
        let data;
      ...
      case 'get-classes': {
        const response = await geminusBaseFetch(config, '/api/config/classes/json');
        const text = await response.text();
        let data;
        try { data = JSON.parse(text); } catch { data = text; }
        return new Response(
      ...
      case 'search-objects': {
        ...
        const response = await geminusBaseFetch(config, `/api/search/quick?query=${encodeURIComponent(query)}`);
        const text = await response.text();
        let data;
        try { data = JSON.parse(text); } catch { data = text; }
        return new Response(
      ...
      case 'get-perspective-tree': {
        ...
        const response = await geminusBaseFetch(config, `/api/perspective/byguid/subtree/json/${encodeURIComponent(perspectiveId)}/${encodeURIComponent(guid)}`);
        const text = await response.text();
        let data;
        try { data = JSON.parse(text); } catch { data = text; }
        return new Response(
      ...
      case 'get-buildings': {
        const response = await geminusBaseFetch(config, '/api/systeminfo/json');
        const text = await response.text();
        let data;
        try { data = JSON.parse(text); } catch { data = null; }
        return new Response(
      ...
      // ── Load full Geminus Base root tree (tries multiple perspective IDs) ──
      case 'get-geminus-base-root': {
        ...
        for (const pid of perspectiveIds) {
          try {
            const resp = await geminusBaseFetch(config, `/api/perspective/root/json/${pid}`);
            if (!resp.ok) {
              console.log(`Geminus Base get-geminus-base-root: perspective ${pid} returned ${resp.status}`);
              continue;
            }
            ...
            if (nodes.length > 0) {
              ...
              console.log(`Geminus Base get-geminus-base-root: perspective ${pid} returned ${nodes.length} root nodes`);
              break;
            }
          } catch (e: any) {
            console.log(`Geminus Base get-geminus-base-root: perspective ${pid} error:`, e.message);
          }
        }

        if (!rootData) {
          return new Response(
            JSON.stringify({ success: false, error: 'No Geminus Base perspective root returned data. All perspective IDs 1-8 failed.' }),
            { headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
          );
        }
      ...
      // ── CRUD operations for Geminus Base objects ──────────────────────
      ...
      case 'create-object': {
        ...
        if (properties && typeof properties === 'object') {
          const mapped = mapPropertiesToGeminusBase(resolved.targetClass, properties);
          objectPayload.properties = mapped;
        }

        console.log('Geminus Base create-object payload:', JSON.stringify(objectPayload));

        try {
          const response = await geminusBaseFetch(config, '/api/object', {
            method: 'POST',
          ...
          if (!response.ok) {
            return new Response(
              JSON.stringify({ success: false, error: `Geminus Base returned ${response.status}`, data }),
              { status: 200, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
            );
          }
      ...
      case 'update-object': {
        ...
        try {
          ...
          const response = await geminusBaseFetch(config, path, {
            method: 'PUT',
      ...
      case 'get-hierarchy': {
        ...
        try {
          ...
          if (buildingFmGuid) {
            ...
            response = await geminusBaseFetch(
              config,
            ...
            console.log('Geminus Base: Loading root perspective tree (no buildingFmGuid)');
            response = await geminusBaseFetch(
              config,
      ...
      case 'sync-object': {
        ...
        try {
          ...
          console.log(`Geminus Base sync-object: ifcType=${ifcType}, category=${syncCategory} -> targetClass=${targetClass}, classId=${resolvedClassId}`);

          // 1. Check if object exists in Geminus Base
          const checkResp = await geminusBaseFetch(config, `/api/object/byguid/json/${encodeURIComponent(fmGuid)}`);
          ...
          console.log(`Geminus Base: byguid response status=${checkResp.status}, keys=${existingObj ? Object.keys(existingObj).join(',') : 'null'}, snippet=${checkText.substring(0, 300)}`);
          ...
          if (objectExists) {
            ...
            // Extract properties from Geminus Base object
            ...
            if (direction === 'push' && syncProps && Object.keys(syncProps).length > 0) {
              // Map properties to Geminus Base field names before pushing
              const mappedUpdateProps = mapPropertiesToGeminusBase(targetClass, syncProps);
              const updatePayload: any = {};
              if (syncName) updatePayload.objectName = syncName;
              updatePayload.properties = mappedUpdateProps;

              const updateResp = await geminusBaseFetch(config, `/api/object/byguid/${encodeURIComponent(fmGuid)}`, {
                method: 'PUT',
                body: JSON.stringify(updatePayload),
              });
              ...
              return new Response(
                JSON.stringify({ 
                  ...
                  targetClass,
                  data: updateData,
                  error: !updateResp.ok ? `Geminus Base returned ${updateResp.status}` : undefined 
                }),
                { headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
              );
            ...
            if (!syncParentGuid || !syncName) {
              return new Response(
                JSON.stringify({ success: false, error: 'Object not found in Geminus Base and parentGuid+name required for creation', action: 'skip' }),
                { status: 200, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
              );
            }

            // Map properties to Geminus Base domain-specific field names
            const mappedCreateProps = mapPropertiesToGeminusBase(targetClass, syncProps || {});
            ...
            const createResp = await geminusBaseFetch(config, '/api/object', {
              method: 'POST',
            ...
            return new Response(
              JSON.stringify({ 
                ...
                direction: 'push',
                data: createData,
                error: !createResp.ok ? `Geminus Base returned ${createResp.status}` : undefined 
              }),
              { headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
            );
          }
      ...
      // ── Ensure full building hierarchy exists in Geminus Base ──────────
      case 'ensure-hierarchy': {
        ...
        try {
          ...
          // 1. Check if building already exists in Geminus Base
          const checkResp = await geminusBaseFetch(config, `/api/object/byguid/json/${encodeURIComponent(buildingFmGuid)}`);
          const checkText = await checkResp.text();
          let existingObj: any = null;
          try { existingObj = JSON.parse(checkText); } catch { existingObj = null; }
          ...
          if (buildingExists) {
            console.log('Geminus Base ensure-hierarchy: Building already exists:', buildingName);
            return new Response(
              JSON.stringify({ success: true, action: 'skipped', message: 'Building already exists in Geminus Base', created, skipped: ['building'] }),
              { headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
            );
          }

          console.log('Geminus Base ensure-hierarchy: Building not found, creating hierarchy for:', buildingName);
          ...
          console.log('Geminus Base ensure-hierarchy: Root perspective GUID:', rootGuid);
          ...
          for (const node of rootNodes) {
            ...
            if (nodeName === fastighetName.toLowerCase().trim() && (nodeClassId === 102 || !nodeClassId)) {
              ...
              console.log('Geminus Base ensure-hierarchy: Found existing Fastighet:', fastighetName, '->', fastighetGuid);
          ...
          if (!fastighetGuid) {
            const fastighetResp = await geminusBaseFetch(config, '/api/object', {
              method: 'POST',
              body: JSON.stringify({ objectName: fastighetName, parentGuid: rootGuid, classId: 102 }),
            });
            const fastighetData = await fastighetResp.json();
            console.log('Geminus Base ensure-hierarchy: Created Fastighet:', fastighetResp.status, JSON.stringify(fastighetData).substring(0, 300));
            ...
            if (!fastighetGuid) {
              ...
              const lookupResp = await geminusBaseFetch(config, '/api/perspective/root/json/8');
              if (lookupResp.ok) {
          ...
          const byggnadResp = await geminusBaseFetch(config, '/api/object', {
            method: 'POST',
          ...
          console.log('Geminus Base ensure-hierarchy: Created Byggnad:', byggnadResp.status, JSON.stringify(byggnadData).substring(0, 300));
          ...
          for (const level of levelArray) {
            try {
              const planResp = await geminusBaseFetch(config, '/api/object', {
                method: 'POST',
              ...
              console.log('Geminus Base ensure-hierarchy: Created Plan:', level.name, planResp.status);
              if (planResp.ok) {
                levelGuids[level.fmGuid] = planData.systemGuid || planData.guid || level.fmGuid;
                created.push(`plan:${level.name}`);
              }
            } catch (e: any) {
              console.error('Geminus Base ensure-hierarchy: Plan creation failed:', level.name, e.message);
            }
          }
          ...
          for (const room of roomArray) {
            ...
            try {
              const roomResp = await geminusBaseFetch(config, '/api/object', {
                method: 'POST',
                body: JSON.stringify({ objectName: room.name || 'Rum', parentGuid, classId: 107, systemGuid: room.fmGuid, targetClass: 'fmo_space_i' }),
              });
              console.log('Geminus Base ensure-hierarchy: Created Room:', room.name, roomResp.status);
              if (roomResp.ok) roomsCreated++;
            } catch (e: any) {
              console.error('Geminus Base ensure-hierarchy: Room creation failed:', room.name, e.message);
            }
          }
          if (roomsCreated > 0) created.push(`${roomsCreated} rum`);

          // 7. Update building_settings with Geminus Base GUID
          try {
            const supabaseUrl = Deno.env.get('SUPABASE_URL') || '';
            const serviceRoleKey = Deno.env.get('SUPABASE_SERVICE_ROLE_KEY') || '';
            if (supabaseUrl && serviceRoleKey) {
              await fetch(`${supabaseUrl}/rest/v1/building_settings?fm_guid=eq.${encodeURIComponent(buildingFmGuid)}`, {
                ...
                body: JSON.stringify({ geminus_base_building_guid: byggnadGuid }),
              });
              console.log('Geminus Base ensure-hierarchy: Updated building_settings with Geminus Base GUID');
            }
          } catch (e: any) {
            console.log('Geminus Base ensure-hierarchy: Failed to update building_settings:', e.message);
          }
          ...
          console.error('Geminus Base ensure-hierarchy error:', error.message);
          return new Response(
            JSON.stringify({ success: false, error: error.message }),
            { status: 200, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
          );
      ...
      case 'proxy': {
        ...
        const response = await geminusBaseFetch(config, apiPath, {
      ...
      // ── Save Geminus Base credentials to api_profiles (uses service role → bypasses RLS) ──
      case 'save-api-config': {
        ...
        try {
          ...
          const payload = {
            geminus_base_api_url: apiUrl.replace(/\/+$/, ''),
            geminus_base_username: username,
            geminus_base_password: password,
            is_default: true,
            name: 'Default',
          };
    ...
    console.error('Geminus Base query error:', error);
    return new Response(
      JSON.stringify({ success: false, error: error.message }),
